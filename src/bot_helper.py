...
def users_stats(user: str) -> Stats:
    ...
    if steam_api.get_users_two_week_stats(user_id) is None:
        two_week_playtime = 0
        two_week_games_string = "And so has not played any games recently"
    else:
        two_week_stats = steam_api.get_users_two_week_stats(user_id)
        two_week_playtime = formatter.format_users_playtime(two_week_stats.two_week_playtime)
        two_week_games_string = f"Recently playing the following games: {formatter.format_list_into_a_string(two_week_stats.two_week_games)}"
